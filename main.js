...
// HOME HERO: storyboard-locked cinematic split
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const curtain = hero && hero.querySelector('[data-hero-layer="curtain"]');
  const blackSurface = hero && hero.querySelector('[data-hero-layer="black"]');
  const openingLines = hero && hero.querySelector('.hero-lines-opening');
  const leftSplitLines = hero && hero.querySelector('.hero-copy-left [data-hero-state="split"]');
  const rightCopy = hero && hero.querySelector('[data-hero-copy="right"]');
  const leftFocus = hero && hero.querySelector('[data-hero-focus="left"]');
  const rightFocus = hero && hero.querySelector('[data-hero-focus="right"]');
  const lsStage = hero && hero.querySelector('[data-hero-copy="ls"]');
  const isolatedL = hero && hero.querySelector('[data-hero-letter="l"]');
  const isolatedS = hero && hero.querySelector('[data-hero-letter="s"]');
  const isolationLeft = isolatedL && isolatedL.parentElement;
  const isolationRight = isolatedS && isolatedS.parentElement;
  const lsLockup = hero && hero.querySelector('.hero-ls-lockup');

  if (!hero || !pin || !curtain || !blackSurface || !openingLines || !leftSplitLines || !rightCopy || !leftFocus || !rightFocus || !lsStage || !isolationLeft || !isolationRight || !lsLockup) return;
  ...
  gsap.set([leftSplitLines, rightCopy, leftFocus, rightFocus, isolationLeft, isolationRight, lsLockup], { autoAlpha: 0 });

  const tl = gsap.timeline({ defaults: { ease: 'none' } });

  tl.addLabel('phase1')
    .to(openingLines, { autoAlpha: 1, duration: 0.8 }, 'phase1')

    .addLabel('phase2', '+=0.25')
    .to(curtain, { xPercent: -95, duration: 1.8, ease: 'power2.inOut' }, 'phase2')
    .to(blackSurface, { autoAlpha: 0.72, duration: 1.4, ease: 'power2.out' }, 'phase2+=0.2')
    .to(openingLines, { autoAlpha: 0, duration: 0.7 }, 'phase2')
    .to(leftSplitLines, { autoAlpha: 1, duration: 0.9 }, 'phase2+=0.35')
    .to(rightCopy, { autoAlpha: 1, duration: 0.9 }, 'phase2+=0.55')

    .addLabel('midHold', '+=0.35')
    .to({}, { duration: 0.65 })

    .addLabel('phase3')
    .to(curtain, { xPercent: -190, duration: 1.2, ease: 'power3.inOut' }, 'phase3')
    .to(blackSurface, { autoAlpha: 1, duration: 1.0 }, 'phase3')
    .to([leftSplitLines, rightCopy], { autoAlpha: 0, duration: 0.7 }, 'phase3+=0.15')
    .to([leftFocus, rightFocus], { autoAlpha: 1, duration: 0.55 }, 'phase3+=0.32')

    .addLabel('phase4', '+=0.5')
    .to([leftFocus, rightFocus], { autoAlpha: 0, duration: 0.6 }, 'phase4')
    .to([isolationLeft, isolationRight], { autoAlpha: 1, duration: 0.55 }, 'phase4+=0.25')

    .addLabel('phase5', '+=0.35')
    .to(isolationLeft, { x: '40vw', y: '-1.5vh', duration: 0.8, ease: 'power2.inOut' }, 'phase5')
    .to(isolationRight, { x: '-22vw', y: '-1.5vh', duration: 0.8, ease: 'power2.inOut' }, 'phase5')
    .to([isolationLeft, isolationRight], { autoAlpha: 0, duration: 0.35 }, 'phase5+=0.65')
    .to(lsLockup, { autoAlpha: 1, duration: 0.45 }, 'phase5+=0.72');
  ...
  const mm = gsap.matchMedia();
  mm.add('(max-width: 900px)', () => {
    gsap.set(curtain, { xPercent: -190 });
    gsap.set(blackSurface, { autoAlpha: 1 });
    gsap.set(openingLines, { autoAlpha: 0 });
    gsap.set([leftSplitLines, rightCopy, leftFocus, rightFocus, isolationLeft, isolationRight], { autoAlpha: 0 });
    gsap.set(lsLockup, { autoAlpha: 1 });
  });
