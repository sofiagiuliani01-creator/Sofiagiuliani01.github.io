...
window.addEventListener('DOMContentLoaded', () => {
  ...
  ScrollTrigger.create({
    ...
    onUpdate: ({ progress }) => {
      ...
      const pullerX = gsap.utils.interpolate(8.1, -13.9, pullerEnter) + gsap.utils.interpolate(0, 2.2, pullerSettle);
      ...
      const walkLegFront = 24.5 * stride * strideMix;
      const walkLegBack = 24.5 * counterStride * strideMix;
      const walkArmFront = -20 * stride * strideMix;
      const walkArmBack = -20 * counterStride * strideMix;
      const walkBob = -6.9 * Math.abs(bodyRhythm) * strideMix;
      const walkHipShift = 7.2 * stride * strideMix;
