...
// GoalArcherSection: sezione cinematografica autonoma con SVG inline e fallback statico.
document.addEventListener('DOMContentLoaded', () => {
  ...
  const PATHS = {
    desktop: {
      ...
      arrow: 'M558 514 C932 526 1306 538 1680 550', flexUp: 'M558 514 C932 494 1306 498 1680 550', flexDown: 'M558 514 C932 548 1306 560 1680 550', flexSmall: 'M558 514 C932 514 1306 523 1680 550', flexEnd: 'M558 514 C932 526 1306 538 1680 550',
      hand: [558, 514], stringRestX: 626, targetX: 955, targetOrigin: '1733 550', archerExit: -1900, arrowFlight: 895, arrowReframe: 495, targetClip: { cx: 1733, cy: 550, rx: 422, ry: 744 }
    },
    mobile: {
      ...
      arrow: 'M566 775 C948 812 1331 856 1714 900', flexUp: 'M566 775 C948 778 1331 820 1714 900', flexDown: 'M566 775 C948 835 1331 880 1714 900', flexSmall: 'M566 775 C948 798 1331 842 1714 900', flexEnd: 'M566 775 C948 812 1331 856 1714 900',
      hand: [566, 775], stringRestX: 726, targetX: 1020, targetOrigin: '1770 900', archerExit: -1900, arrowFlight: 900, arrowReframe: 500, targetClip: { cx: 1770, cy: 900, rx: 454, ry: 770 }
    }
  };
  ...
  const requireNode = (root, selector, label = selector) => {
    const node = root.querySelector(selector);
    if (!node) throw new Error(`SVG incompleto: manca ${label}`);
    return node;
  };
  ...
  const collectRig = (archerSvg, targetSvg) => ({
    upper: requireNode(archerSvg, '[data-goal-part="upper"]'),
    head: requireNode(archerSvg, '[data-goal-part="head"]'),
    lower: requireNode(archerSvg, '[data-goal-part="lower"]'),
    bow: requireNode(archerSvg, '[data-goal-part="bow"]'),
    string: requireNode(archerSvg, '[data-goal-part="string"]'),
    stringTop: requireNode(archerSvg, '[data-goal-part="string-top"]'),
    stringBottom: requireNode(archerSvg, '[data-goal-part="string-bottom"]'),
    arrow: requireNode(archerSvg, '[data-goal-part="arrow"]'),
    arrowShaft: requireNode(archerSvg, '[data-goal-part="arrow-shaft"]'),
    target: requireNode(targetSvg, '[data-goal-part="target"]'),
    darkArrow: requireNode(targetSvg, '[data-goal-part="arrow-dark"]')
  });
  ...
  const applyStaticState = ({ cfg, archerSvg, targetSvg }) => {
    ...
    rig.bow.setAttribute('d', cfg.bowTense);
    rig.stringTop.setAttribute('x2', cfg.hand[0]);
    rig.stringTop.setAttribute('y2', cfg.hand[1]);
    rig.stringBottom.setAttribute('x2', cfg.hand[0]);
    rig.stringBottom.setAttribute('y2', cfg.hand[1]);
    rig.arrowShaft.setAttribute('d', cfg.flexEnd);
    rig.darkArrow.setAttribute('d', cfg.flexEnd);
    rig.darkArrow.setAttribute('opacity', '1');
  };
  ...
  const buildTimeline = ({ cfg, archerSvg, targetSvg, mobile = false, reduced = false }, gsapApi) => {
    const rig = collectRig(archerSvg, targetSvg);
    const chars = gsapApi.utils.toArray(section.querySelectorAll('.goal-scene__char'));
    const textBlock = section.querySelector('[data-goal-text]');
    if (!textBlock) throw new Error('Sezione obiettivo incompleta: manca il blocco di testo');

    if (reduced) {
      applyStaticState({ cfg, archerSvg, targetSvg });
      gsapApi.set(chars, { opacity: 1, x: 0, skewX: 0, scaleY: 1 });
      gsapApi.set(textBlock, { x: 0, y: 0 });
      return null;
    }
    ...
    timeline.to({ clock: 0 }, { clock: 1, duration: 1 }, 0)
      ...
      .to(rig.head, { rotation: 0, x: 0, y: 0, ease: 'power2.out', duration: 0.08 }, 0)
      .to([rig.bow, rig.string], { rotation: -2, x: 0, y: 0, ease: 'power2.out', duration: 0.06 }, 0)
      .to([rig.bow, rig.string], { rotation: 0, duration: 0.02 }, 0.06)
      .to(rig.arrow, { rotation: 0, x: 0, y: 0, ease: 'power2.out', duration: 0.08 }, 0)
      .to(rig.upper, { rotation: 0.55, y: -2, yoyo: true, repeat: 1, duration: 0.11 }, 0.08)
      .to(rig.bow, { attr: { d: cfg.bowTense }, duration: 0.22 }, 0.08)
    ...
    if (!mobile) {
      timeline
        .to(textBlock, { x: 0, y: 0, duration: 0.120 }, 0.335)
        .to(chars, { opacity: 1, x: 0, skewX: 0, scaleY: 1, stagger: { amount: 0.085, from: 'end' }, duration: 0.035, ease: 'power3.out' }, 0.335);
    } else {
      timeline.duration(4.037);
    }
  ...
  if (reduced) section.classList.add('is-goal-static');
