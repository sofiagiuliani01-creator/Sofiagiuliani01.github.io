...
window.addEventListener('DOMContentLoaded', () => {
  ...
  ScrollTrigger.create({
    ...
    onUpdate: ({ progress }) => {
      ...
      const walkPhase = walkDrive * Math.PI * 11.2;
      const stride = Math.sin(walkPhase);
      const counterStride = Math.sin(walkPhase + Math.PI);
      const bodyRhythm = Math.sin(walkPhase * 2);
      const strideMix = gsap.utils.interpolate(1.18, 0.45, walkEaseOut);
      const walkLegFront = 18.5 * stride * strideMix;
      const walkLegBack = 18.5 * counterStride * strideMix;
      const walkArmFront = -16 * stride * strideMix;
      const walkArmBack = -16 * counterStride * strideMix;
      const walkBob = -5.8 * Math.abs(bodyRhythm) * strideMix;
      const walkHipShift = 6.2 * stride * strideMix;
      const walkTorsoLean = gsap.utils.interpolate(-9.5, -3.1, walkEaseOut) + (1.7 * stride * strideMix);
